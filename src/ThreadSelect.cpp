...
#include "FMain.h"  // Убрать после переноса генерации экселя в этот класс
...
void __fastcall ThreadSelect::Execute()
{
    if (!ThreadOraSession->Connected) {
        ...
        ThreadMessage = "Произошла ошибка. База данных недоступна.";
        this->Terminate();
    }
    ...
    if (!this->Terminated && sQueryText != "") {   // Если задан первый запрос
        ...
        try {
            ...
            ThreadMessage = "Ошибка при попытке выполнить запрос.\n" + e.Message;
        }
 ...
 /*   while (OraQuery->Executing()) {
    ...
    // !!!!!!!!!!!!!!!!!!!!!!!! Этот блок наверно можно убрать
    /*if (!this->Terminated && OraQueryMain == NULL) {
//------------------------------------------------------------------------------------------------------------------------------
...
void __fastcall ThreadSelect::ExportToWordTemplate(TOraQuery *QueryMerge, TOraQuery *QueryFormFields)
{
    ...
    try {
        ...
        ThreadMessage = "Неудалось создать экземпляр приложения Microsoft Word."
    ...
    try {
        ...
        VarClear(Document);
        ThreadMessage = "Неудалось открыть шаблон " + TemplateFullName +
            "\nПожалуйста, обратитесь к системному администратору.\n" + e.Message;
        throw Exception(ThreadMessage);
    }
        ...
        for( ; !QueryFormFields->Eof; QueryFormFields->Next()) { // Цикл по полям FormFields
            ...
            if (bFilterExist) { // Если во вспомогательном запросе больше 1 строки, то применяем фильтр
                try {
                    ...
                    QueryMerge->Filtered = false;
                    ThreadStatus = WM_THREAD_ERROR_IN_PROCESS;
                    ThreadMessage = "Проверьте корректность параметров фильтра в параметрах экспорта или обратитесь к системному администратору.\n" + e.Message;
                    break;
                }
            ...
            for (int i = 0; i < FormFieldsCount ; i++)  { 		// Перебираем FormFields, подставляем соответствующие значения из QueryFormFields
                ...
                try {
                    ...
                    ThreadMessage = "Возникла ошибка при замене полей FormFields в шаблоне """ + TemplateFullName + """"
                        ", поле """ + FormFieldName + """."
            ...
            if (QueryMerge->RecordCount > 0) {
                ...
                try {
                    ...
                    ThreadStatus = WM_THREAD_ERROR_IN_PROCESS;
                    ThreadMessage = "В процессе слияния документа с источником данных произошла ошибка."
                        "\nОбратитесь к системному администратору."
                        "\n" + e.Message;
                    break;
                }
...
void __fastcall ThreadSelect::ExportToExcelTemplate(TOraQuery *QueryTable, TOraQuery *QueryFields)
{
    ...
    try {
        ...
        CoUninitialize();
        ThreadMessage = "Ошибка при открытии файла-шаблона " + TemplateFullName + ".\nОбратитесь к системному администратору.";
        throw Exception(ThreadMessage);
    }
    ...
    if (DstFileName == "") {        // Просто открываем документ, если имя файла-результата не задано
        ...
        try {
            ...
            ThreadMessage = "Ошибка при сохранении результата в файл " + DstFileName + ".\n" + e.Message;
            throw Exception(ThreadMessage);
        }
    }
...
void __fastcall ThreadSelect::ExportToDBF(TOraQuery *OraQuery)
{
    ...
    if (n > 0) {   // Формируем список полей для экспорта в DBF ("Имя;Тип;Длина;Длина дробной части")
        ...
        ThreadMessage = "Не задан список полей в параметрах экспорта."
            "\nПожалуйста, обратитесь к системному администратору.";
        throw Exception(ThreadMessage);
    }
    ...
    if (ListFields->Count > OraQuery->FieldCount && !param_dbase.fAllowUnassignedFields) {
        ThreadMessage = "Количество требуемых полей превышает количество полей в источнике данных."
            "\nПожалуйста, обратитесь к системному администратору.";
        throw Exception(ThreadMessage);
    }
